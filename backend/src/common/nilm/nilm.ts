...
export class NILMEngine {
    ...
    private handleOn(event: PowerEvent) {
        ...
        // 🔥 Ignore low-confidence matches (prevents noise pollution)
    ...
    private handleOff(event: PowerEvent) {
        ...
        if (matchIndex === -1) {
            // 🔥 Sort for faster matching (big appliances first)
            this.active.sort((a, b) => b.power - a.power);
            ...
            if (combo.length > 0) {
                combo
                    .sort((a, b) => b - a)
                    .forEach(idx => this.closeAppliance(idx, event.timestamp));
                return;
            }
    ...
    // 🔥 FIXED VERSION (no blocking)
    private findCombination(target: number): number[] {
        // 🔥 Hard safety limit (prevents exponential explosion)
        if (this.active.length > 15) return [];
